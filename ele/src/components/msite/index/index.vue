<template>
	<section>
		<header class="ele-header">
			<section class="ele-header-container">
				<div data-aria = "hhhhh" class="ele-header-aria" @click="$emit('openSearch')">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 31" class="icon01">
					...
					<span class="ele-header-text">{{isExit_location?city:city_info.name}}</span>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 8" class="icon02"><path fill="#FFF" fill-rule="evenodd" d="M5.588 6.588c.78.78 2.04.784 2.824 0l5.176-5.176c.78-.78.517-1.412-.582-1.412H.994C-.107 0-.372.628.412 1.412l5.176 5.176z" data-spm-anchor-id="a2ogi.12117543.0.i3"></path></svg>
				</div>
			</section>
		</header>
		<div class="search-warpper" ref='search' style="position: sticky; top: 0px; z-index: 999;" >
			<div class="search">
		...
		<!--遮罩层-->
       	<div class="filtermodal" v-show="filter_Model" @touchmove.prevent @click = "closeModel()"></div>
		<div class="shoplist-title" id="shoplist-title" v-if='shoplist_title_show'>推荐商家</div>
...
<script>
	...
	export default{
		...
		computed:{
			...
			filter_data:function(){ return this.$store.state.filter.filterParam; },
		},
		watch:{
			filter_Model:function(cur_val,old_val){
				this.$nextTick(()=>{
					this.shoplist_title = this.shoplist && document.getElementById('shoplist-title');
					this.html.scrollTop = (this.html.scrollTop < this.shoplist_title.offsetHeight + this.shoplist_title.offsetTop && cur_val) ? (this.shoplist_title.offsetHeight + this.shoplist_title.offsetTop):this.html.scrollTop;
				})
			},
		...
		methods:{
			...
			controlModel:function(data){						//监听子传父事件
			...
			reShoplist:function(){					//重新加载商品
				this.shoplist = [];					//置空商品列表 !!
				this.offset = 0;
				this.load_data = true;
				this.getShoplist();
			},
			getPosition:function(){								//获取经纬度
				this.city = '正在定位';								
				this.load_location = true;
				return new Promise((resolve,reject) => {			//调用promise ES6 异步处理机制
					/*配置属性*/
					let options = {									//配置属性
						timeout:3000,								//设置请求时间
						enableHighAccuracy: true					//高精度定位
					}
					if(navigator.geolocation){						//是否开启定位 或者浏览器是否支持
						...
						reject("no thanks");						//不存在返回信息 catch接收
				})
			},
			setPosition:function(){								//判断是否成功获取并设置经纬度
				this.getPosition().then(data =>{				//调用getPosition 函数 成功时接收信息
					//发起axios请求 PC端获取不到地址（暂时不用管） 
					this.getCity(data.latitude,data.longitude)
				}).catch(error =>{								//调用getPosition 函数 失败时接收信息
//					this.JugdePosition();//失败时判断是否存在session
					this.getLs().then((location) =>{
			...
			getCity:function(latitude,longitude){				//获取城市传递给Vuex
				//axios 基于promise 参考上方注释
				axios.get(`restapi/bgs/poi/reverse_geo_coding?latitude=${latitude}&longitude=${longitude}`).then((city) => {	 				
					//`..${变量名}..`ES6 字符串模板 
...
<style scoped>
	...
	.search:before,.index-search:after{
		content: "";
		position: absolute;
		width: 100%;
		height: 2px;
	...
	.shoplist-title:before{ margin-right: 3.466667vw; }
	.shoplist-title:after{ margin-left: 3.466667vw; }
	.spinner{
		height: 1.333333rem;
		display: flex;
